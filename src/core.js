...
const settings = Object.assign({ diff: 1, quality: 1, sens: 1, fov: 90, vol: 0.6, view: 'fpp', op: 'wolf', name: '干员' }, loadJSON('delta3d.settings'));
function saveSettings() { try { localStorage.setItem('delta3d.settings', JSON.stringify(settings)); } catch (e) { } }
...
function applyQuality() {
  ...
  if (scene.fog) scene.fog.far = [420, 600, 850][q];
  resize();
}
